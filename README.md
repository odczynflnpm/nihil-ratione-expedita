...
```bash
npm install @odczynflnpm/nihil-ratione-expedita
```
...
```javascript
require ('@odczynflnpm/nihil-ratione-expedita').nice // .nice for unsafe String extensions
...
```javascript
import { green, inverse, bgLightCyan, underline, dim } from '@odczynflnpm/nihil-ratione-expedita'
```
```javascript
const { green, inverse, bgLightCyan, underline, dim } = require ('@odczynflnpm/nihil-ratione-expedita')
```
...
```javascript
import { @odczynflnpm/nihil-ratione-expedita, ParsedSpan } from '@odczynflnpm/nihil-ratione-expedita' // along with type definitions
```
```javascript
import @odczynflnpm/nihil-ratione-expedita from '@odczynflnpm/nihil-ratione-expedita'
```

### Nice Mode (not recommended)

```javascript
const ansi = require ('@odczynflnpm/nihil-ratione-expedita').nice
```

The `('@odczynflnpm/nihil-ratione-expedita').nice` export defines styling APIs on the `String` prototype directly. It uses an ad-hoc DSL (sort of) for infix-style string coloring. The `nice` is convenient, but not safe, avoid using it in public modules, as it alters global objects, and that might cause potential hard-to-debug compatibility issues.
...
```javascript
const { names } = require ('@odczynflnpm/nihil-ratione-expedita')
...
```javascript
const { strip } = require ('@odczynflnpm/nihil-ratione-expedita')
...
```javascript
const { isEscaped, green } = require ('@odczynflnpm/nihil-ratione-expedita')
...
```javascript
const { parse } = require ('@odczynflnpm/nihil-ratione-expedita')
...
```javascript
const ansi = require ('@odczynflnpm/nihil-ratione-expedita')
...
```javascript
const { bgGreen, red, parse } = require ('@odczynflnpm/nihil-ratione-expedita')
...
## Projects That Use `@odczynflnpm/nihil-ratione-expedita`
